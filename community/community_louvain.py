...
def partition_at_level(dendrogram, level):
    ...
    ------
    KeyError
       If the dendrogram is not well formed or the level is too high     ?# how high???
    See Also
    --------
    ...
    >>> for level in range(len(dendrogram) - 1) :
            # *** error :AttributeError: 'list' object has no attribute 'partition_at_level'
...
def generate_dendrogram(graph,
    ...
    n_seconds = (datetime.now() - t1).seconds
    print("iter :{}  \tmodularity :{:.7f}\t  time :{} s".format(iters,new_mod,n_seconds))

    if save_iter_file is not None:
        with open(save_iter_file+'_iter{}_seconds{}'.format(iters,n_seconds),'w') as f:
